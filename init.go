...
var initCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fail("Error getting current directory: %s", err)
		}
		...
		if err == nil {
			fail("Sto profile already exists in this directory: %q", stoFilePath)
		}
		if err != nil && !errors.Is(err, os.ErrNotExist) {
			fail("Error checking for existing Sto profile: %s", err)
		}

		file, err := os.Create(stoFilePath)
		if err != nil {
			fail("Error creating Sto file at %q: %s", stoFilePath, err)
		}
		file.Close()

		if _, err := os.Stat(statePath); err != nil {
			if !errors.Is(err, os.ErrNotExist) {
				fail("Error checking for state directory at %q: %s", statePath, err)
			}
			if err := os.MkdirAll(statePath, 0700); err != nil {
				fail("Error creating state directory at %q: %s", statePath, err)
			}
		}

		file, err = os.OpenFile(fmt.Sprintf("%s/current-profile", statePath), os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0600)
		if err != nil {
			fail("Error creating state file at %q: %s", statePath, err)
		}
